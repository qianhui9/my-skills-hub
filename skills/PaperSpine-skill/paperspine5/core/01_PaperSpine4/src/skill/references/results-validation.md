...
**test a promise the paper will make** — a claim derived from
`confirmed_contribution.md` and later surfaced in the Abstract or Introduction
— and either confirm it or honestly fail it. If a number appears in
...
`results_validation.md` forces the mapping to be explicit during planning,
*before* you write Results prose, so each subsection is born already tied to a
confirmed claim, an evidence locus, and a stated limit on interpretation.
...
3. For each planned major Results subsection, fill the row before prose. If a subsection cannot be
