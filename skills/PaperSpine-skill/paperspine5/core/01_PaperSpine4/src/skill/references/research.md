...
gaps, and produce user-confirmable contribution and motivation options.
Research must complete before the user confirms the semantic contract.
...
## Stage 2 — Research Lenses

Apply all three lenses below. In balanced mode, one Agent may handle them in a
single coherent research pass and write the three outputs directly. Strict or
pro mode may read `phase-contracts.md` and launch three independent specialists
with separate receipts when independence adds real value.
...
Produce `style_profile.md`, `contribution_options_after_research.md`, and
`motivation_options_after_research.md`. Each motivation option must identify the
contribution option it supports. Stop for user confirmation; route to
`references/semantic-confirmation.md`. Write neither confirmed artifact until
the user chooses, revises, combines, or writes their own contract.
...
- `confirmed_contribution.md` and `confirmed_motivation.md` (after semantic confirmation)
