...
- `paper_rewriting_output/confirmed_contribution.md` must be explicitly confirmed by the user and pass `contribution_check.py` **before** any section drafting, blueprinting, Results planning, or rewriting begins.
...
1. Research 3-5 defensible alternatives in `contribution_options_after_research.md`; do not silently choose one for the user.
2. After the user chooses or revises an option, write `confirmed_contribution.md` with all four sections and resolve `Evidence missing` honestly before locking the Core statement.
3. Run `python src/scripts/contribution_check.py paper_rewriting_output` until it passes (exit 0).
4. Confirm an aligned motivation, then proceed to Results planning, section blueprints, and writing — each tracing back to the Core contribution.
5. Re-run the check whenever the claim or evidence changes; a shifted contribution invalidates downstream planning and prose.
